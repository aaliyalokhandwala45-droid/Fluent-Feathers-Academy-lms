...
async function initializeDatabase() {
  ...
  try {
    ...
    const checkColumn = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'is_active'`);
    if (checkColumn.rows.length === 0) await client.query(`ALTER TABLE students ADD COLUMN is_active BOOLEAN DEFAULT true`);
