...
function getHomeworkFeedbackEmail(data) {
  const { studentName, parentName, grade, comments, fileName, materialType } = data;
  const typeLabel = materialType === 'Classwork' ? 'Classwork' : 'Homework';
  ...
  try {
    ...
    const emailHtml = `
      <div style="font-family:Arial,sans-serif; color:#333; line-height:1.5;">
        <h2 style="color:#B05D9E;">Hello ${student.parent_name || 'Parent'},</h2>
        <p>Please go to the <strong>Profile / Expectations</strong> section in the parent portal and scroll down to fill out your expectations for <strong>${student.name}</strong>.</p>
        <p style="margin:16px 0;">
          <a href="${portalLink}" style="display:inline-block;padding:12px 20px;background:#667eea;color:white;border-radius:8px;text-decoration:none;">Open Parent Portal</a>
        </p>
        <p style="margin:0;">Once you submit your goals and preferences, the teacher can tailor the learning plan to your child.</p>
      </div>
    `;
...
app.post('/api/live-points/award', async (req, res) => {
  try {
    ...
    if (totalPoints % 10 === 0 && prevTotal < totalPoints) {
      const threshold = totalPoints;
      const badgeName = `⭐ ${threshold} Class Points!`;
      const badgeDesc = `Earned ${threshold} class points in live classes!`;
      const badgeType = `class_points_${threshold}`;
      const awarded = await awardBadge(student_id, badgeType, badgeName, badgeDesc);
      if (awarded) badgeAwarded = badgeName;
    }
